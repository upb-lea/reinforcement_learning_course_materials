%calc_forest_MC_off_policy_WIS_prediction_example: Calculating exemplary prediction trajectories for %off-policy Monte Carlo learning with weighted importance sampling 
%
...
%% Initals 
%target policy (optimal policy from MDP solution for alpha = 0.2, gamma = 0.8)
p(1,1) = 0.0; %probability of u=cut at state = 1
p(1,2) = 1.0; %probability of u=wait at state = 1

p(2,1) = 1.0; %probability of u=cutt at state = 2
p(2,2) = 0.0; %probability of u=wait at state = 2

p(3,1) = 1.0; %probability of u=cut at state = 3
p(3,2) = 0.0; %probability of u=wait at state = 3
...
set(fig.sp(end), 'ylim', [0 4], 'xlim', [0 num_epsiodes]);
 ...
 if save_plot ==1
        FigName = ['Forest_Tree_MC_Off_Policy_Prediction_WIS.pdf'];
