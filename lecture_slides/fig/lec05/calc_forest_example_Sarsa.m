%calc_forest_example_Sarsa: Calculating exemplary  control trajectories for Sarsa reinforcement learning applied to the forst tree example
%
...
num_epsiodes = 20000; %number of episodes per trial
%% Initals 
%policy (fifty-fifty)
...
for ee = 1:num_trials
    ...
    for ii = 1:num_epsiodes
        alpha_TD = 0.5*1/((ii)^(1/3)); %adaptive alpha
        ...
        while x~= 4 %terminate state?
            ...
            if x_next ~= 4 %Is next step terminal?
                u_next = Forest_action(x_next, p);
                q(x,u) = q(x,u) + alpha_TD*(r + gamma*q(x_next,u_next) - q(x,u));
            else
                u_next = 0;
...
set(fig.sp(end), 'xlim', [0 num_epsiodes]);
 ...
 if save_plot ==1
        FigName = ['Forest_Tree_Sarsa_alpha_adapt.png'];
