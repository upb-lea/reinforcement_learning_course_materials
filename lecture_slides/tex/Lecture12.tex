...
\begin{itemize}
	...
	\begin{enumerate}
		\item Both  share a mutual parameter set $\bm{\theta}$ (e.g. artifical neural network with multiple outputs).\pause
...
\begin{split}
	...
	\onslide<2->{\Leftrightarrow\quad \nabla_{\bm{\theta}}\ln \pi(u|\bm{x},\bm{\theta}) &= \nabla_{\bm{\theta}}\left(\bm{\theta}\T\tilde{\bm{x}}(\bm{x}, u) - \sum_i \bm{\theta}\T\tilde{\bm{x}}(\bm{x}, i)\right)\\}
...
\item Bias problem applies as in the stochastic case and can be compensated using compatible function approximation w.r.t to $\hat{q}$.\pause
...
\begin{itemize}
	...
	\begin{itemize}
		\item The environment itself is sufficiently noisey (random impacts, measurement noise).\pause
...
\begin{itemize}
	\item \href{https://arxiv.org/pdf/1509.02971.pdf}{Deep Deterministic Policy Gradient (DDPG) Algorithms}
	\item \href{https://arxiv.org/pdf/1502.05477.pdf}{Trust Region Policy Optimization (TRPO)}
	\item \href{https://arxiv.org/abs/1707.06347}{Proximal Policy Optimization (PPO)}
	\item \href{https://arxiv.org/pdf/1611.01224.pdf}{Actor Critic with Experience Replay (ACER)}
	\item \href{https://arxiv.org/pdf/1801.01290.pdf}{Soft Actor-Critic (SAC)}
	\item ....
\end{itemize}
\vspace{0.5cm}\pause
Remarks: 
