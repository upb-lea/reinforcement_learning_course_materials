...
\begin{itemize}
 ...
 \end{equation*}
	...
	\item In contrast, if $\bm{u}\in\mathcal{U}\subset\mathbb{R}^m$ is a (quasi-)continuous variable solving $\max_{\bm{u}} \hat{q}(\bm{x}', \bm{u}, \bm{w})$  requires an \hl{own optimization routine} which is computational expensive if we use nonlinear function approximation. 
\end{itemize}
}
...
\begin{itemize}
	...
	\end{equation}
	\item Hence, we can obtain explicit $Q$-learning targets for continuous actions when using a deterministic policy. For improving the policy we reuse the deterministic policy gradient theorem in an off-policy fashion
	\begin{equation}
	\nabla_{\bm{\theta}} J(\bm{\theta}) = \El{\nabla_{\bm{\theta}} \bm{\mu}(\bm{x},\bm{\theta}) \nabla_{\bm{u}} q(\bm{x},\bm{u})\vert_{\bm{u}=\bm{\mu}(\bm{x})}}{\bm{x}\sim\rho^{\bm{b}}}
\end{equation}
where $\El{\cdot}{\bm{x}\sim\rho^{\bm{b}}}$ is the expected value w.r.t. the state distribution
\begin{equation}
	\rho^{\bm{b}}(\bm{x}')=\int_\mathcal{X}\sum_{k=1}^\infty\gamma^{k-1}p_1(\bm{x})p(\bm{x}\rightarrow\bm{x}',k,\bm{b})
\end{equation}
given a behavior policy $\bm{b}(\bm{u}|\bm{x})$ and starting state density $p_1(\bm{x})$.
\item Above, $p(\bm{x}\rightarrow\bm{x}',k,\bm{b})$ is the density at state $\bm{x}'$ after transitioning for $k$ time steps from a starting state $\bm{x}$ following $\bm{b}(\bm{u}|\bm{x})$.
\end{itemize}
}
...
\begin{itemize}
	\item We store $\left\langle \bm{x}, u, r, \bm{x}'\right\rangle$ in $\bm{\mathcal{D}}$ after each transition step.
	...
	\begin{equation}
	\label{eq:MSBE_DDPG}
		\mathcal{L}(\bm{w}) = \El{\left[\left(r+ \gamma q(\bm{x}',\bm{\mu}(\bm{x}',\bm{\theta}),\bm{w})\right) - q(\bm{x},\bm{u},\bm{w}) \right]^2}{\left\langle \bm{x}, u, r, \bm{x}'\right\rangle\sim\bm{\mathcal{D}}} .
	\end{equation}
\end{itemize}
}
...
\Init{weights $\bm{w}=\bm{w}^{-}\in\mathbb{R}^{\zeta}$ and $\bm{\theta}=\bm{\theta}^{-}\in\mathbb{R}^d$ arbitrarily, memory $\bm{\mathcal{D}}$}
 \For{$j=1,2,\ldots,$ episodes}{
		...
		\For{$k=0,1,\ldots, T-1$ time steps}{
			...
			fit $\bm{w}$ on loss $\mathcal{L}(\bm{w})=[y_i - \hat{q}(\bm{x}_i, \bm{u}_i, \bm{w})]^2_{\bm{\mathcal{D}}_b}$ with step size $\alpha_{w}$\;
			$\bm{\theta} \leftarrow \bm{\theta} + \alpha_{\theta} [\nabla_{\bm{\theta}}\bm{\mu}(\bm{x}_k,\bm{\theta})\nabla_{\bm{u}}\hat{q}(\bm{x}_k, \bm{u}_k, \bm{w})\vert_{\bm{u}=\bm{\mu}(\bm{x})}]_{\bm{\mathcal{D}}_b}$\; 
...
\begin{itemize}
	...
	\begin{equation}
\begin{aligned}
\tilde{\bm{\theta}} &= \bm{\theta} + \frac{\alpha}{Z_1} \El{\nabla_{\bm{\theta}} \pi_{\bm{\theta}}(\bm{x}) \nabla_{\bm{u}} \hat{q}_{\bm{w}}(\bm{x},\bm{u})|_{\bm{u}=\bm{\pi}_{\bm{\theta}}(\bm{x})}}{\bm{x} \sim \rho_{\bm{\pi}}},\\
\bm{\theta}^* &= \bm{\theta} + \frac{\alpha}{Z_2} \El{\nabla_{\bm{\theta}} \pi_{\bm{\theta}}(\bm{x}) \nabla_{\bm{u}} q^{\bm{\pi}}(\bm{x},\bm{u})|_{\bm{u}=\bm{\pi}_{\bm{\theta}}(\bm{x})}}{\bm{x} \sim \rho_{\bm{\pi}}},
\end{aligned}
\end{equation} 
...
\begin{itemize}
	...
	\begin{equation}
		\hat{q}_{\bm{w}}(\bm{x}, \bm{u}) = r + \gamma \E{\hat{q}_{\bm{w}}(\bm{x}', \bm{u}')} - \tilde{\delta}(\bm{x}, \bm{u}).
	\end{equation}
	\item Although this error might be considered small per update step, it may accumulate over future steps if biased:
	\begin{align}
		\hat{q}_{\bm{w}}(\bm{x}_k, \bm{u}_k) &= r_k + \gamma \E{r_{k+1} + \gamma \E{\hat{q}_{\bm{w}}(\bm{x}_{k+2}, \bm{u}_{k+2})} - \tilde{\delta}_{k+1}} - \tilde{\delta}_{k}\notag\\
		&=\ldots\\
		&=\El{\sum_{k=1}^T\gamma^{i-k}(r_i-\tilde{\delta}_{i})}{\bm{x}_i\sim\rho_{\bm{\pi}}, \bm{u}_i\sim\bm{\pi}}\notag.
	\end{align}
...
\begin{itemize}
	...
	\begin{equation}
		y = r + \gamma \hat{q}_{\bm{w}^{-}}(\bm{x}', \bm{\mu}_{\bm{\theta}^{-}}+\bm{\epsilon}).
	\end{equation}
...
\Init{weights $\bm{w}=\bm{w}_{1}^{-}=\bm{w}_{2}^{-}\in\mathbb{R}^{\zeta}$ and $\bm{\theta}=\bm{\theta}^{-}\in\mathbb{R}^d$ arbitrarily, memory $\bm{\mathcal{D}}$}
 \For{$j=1,2,\ldots,$ episodes}{
		...
		\For{$k=0,1,\ldots, T-1$ time steps}{
			...
			fit $\bm{w}_{l}$ on loss $\mathcal{L}(\bm{w}_{l})=[y_i - \hat{q}(\bm{x}_i, \bm{u}_i, \bm{w}_{l})]^2_{\bm{\mathcal{D}}_b}$ with step size $\alpha_{w}$ $\forall l$\;
			\If{$k \mod k_w=0$}{
					$\bm{\theta} \leftarrow \bm{\theta} + \alpha_{\theta} [\nabla_{\bm{\theta}}\bm{\mu}(\bm{x}_k,\bm{\theta})\nabla_{\bm{u}}\hat{q}(\bm{x}_k, \bm{u}_k, \bm{w}_1)\vert_{\bm{u}=\bm{\mu}(\bm{x})}]_{\bm{\mathcal{D}}_b}$\; 
...
\begin{itemize}
	...
	\item First, we rewrite the performance metric \eqref{eq:performance_metric_episodic} to obtain
		\begin{equation}
		J_{\pi} =\El{\sum_{k=0}^\infty \gamma^k r_k}{\bm{x}\sim p(\bm{x}_{k+1}|\bm{x}_{k}, \bm{u}_{k}), \bm{u}_k \sim \pi(\bm{u}|\bm{x})}.
	...
	\item Using the advantage $a_\pi(\bm{x},\bm{u})= q_{\pi}(\bm{x},\bm{u})-v_{\pi}(\bm{x})$ we can calculate the performance of an updated policy $\bm{\pi}\rightarrow\tilde{\bm{\pi}}$\footnote{proof from: S. Kakade and J. Langford, \textit{Approximately optimal approximate reinforcement learning}, ICML, vol. 2, pp 267-274, 2002}:
		...
		J_{\tilde{\pi}} =J_{\pi} + \int_\mathcal{X}\rho^{\tilde{\pi}}(\bm{x})\int_{\mathcal{U}} \tilde{\pi}(\bm{u}|\bm{x})a_\pi(\bm{x},\bm{u}).
	\end{equation}\vspace{-0.5cm}
...
\begin{itemize}
	...
	\begin{equation}
		\mathcal{L}_{\pi}(\tilde{\pi}) =J_{\pi} + \int_\mathcal{X}\rho^{\pi}(\bm{x})\int_{\mathcal{U}} \tilde{\pi}(\bm{u}|\bm{x})a_\pi(\bm{x},\bm{u})
	\end{equation}
	where $\rho^{\pi}(\bm{x})$ is used instead of $\rho^{\tilde{\pi}}(\bm{x})$, i.e., neglecting the state distribution change due to a policy update.
...
\begin{itemize}
	...
	with 
	\begin{equation*}
\overline{D}_{\mathrm{KL}}(\bm{\theta}_k, \bm{\theta})=\overline{D}_{\mathrm{KL}}(\pi_{\bm{\theta}_k}, \pi_{\bm{\theta}})=\El{D_{\mathrm{KL}}(\pi_{\bm{\theta}_k}(\cdot|\bm{x}) \parallel \pi_{\bm{\theta}}(\cdot|\bm{x}))}{\bm{x} \sim \pi_{\bm{\theta}_k}}.
	\end{equation*}
...
\begin{itemize}
	...
	\begin{equation}
		\max_{\bm{\theta}}  \, \mathcal{L}_{\bm{\theta}_k}(\bm{\theta})= \max_{\bm{\theta}} \, J_{\pi_k} + \int_\mathcal{X}\rho^{\pi_k}(\bm{x})\int_{\mathcal{U}} \pi_{\bm{\theta}}(\bm{u}|\bm{x})a_{\pi_k}(\bm{x},\bm{u}).
	\end{equation}
	\item The first term $J_{\pi_k}$ can be dropped, since it is irrelevant for the optimization result (constant).
	\item Using samples we can approximate $\int_\mathcal{X}\rho^{\pi_k}(\bm{x})\approx\frac{1}{1-\gamma}\El{\cdot}{\bm{x}\sim\rho^{\pi_k}}$.
	\item Moreover, $\int_{\mathcal{U}} \pi_{\bm{\theta}}(\bm{u}|\bm{x})a_{\pi_k}(\bm{x},\bm{u})\approx\El{\frac{\pi_{\bm{\theta}}(\bm{u}|\bm{x})}{\pi_{\bm{\theta}_k}(\bm{u}|\bm{x})}a_{\pi_k}(\bm{x},\bm{u})}{\bm{u}\sim\pi_k}$ is also approximated applying importance sampling based on data from the old policy.
	\item Hence, the sampled objective is
	\begin{equation}
		\max_{\bm{\theta}} \,\El{\frac{\pi_{\bm{\theta}}(\bm{u}|\bm{x})}{\pi_{\bm{\theta}_k}(\bm{u}|\bm{x})}a_{\pi_k}(\bm{x},\bm{u})}{\bm{x}\sim\rho^{\pi_k}, \bm{u}\sim\pi_k}.
	\end{equation}
\end{itemize}
}
...
\begin{itemize}
	...
	\begin{split}
		 &\bm{\theta}_{k+1} = \argmax_{\bm{\theta}} \,\El{\frac{\pi_{\bm{\theta}}(\bm{u}|\bm{x})}{\pi_{\bm{\theta}_k}(\bm{u}|\bm{x})}a_{\pi_k}(\bm{x},\bm{u})}{\bm{x}\sim\rho^{\pi_k}, \bm{u}\sim\pi_k},\\
		 \mbox{s.t.} \quad\quad  & \El{D_{\mathrm{KL}}(\pi_{\bm{\theta}_k}(\cdot|\bm{x}) \parallel \pi_{\bm{\theta}}(\cdot|\bm{x}))}{\bm{x} \sim \pi_{\bm{\theta}_k}}\leq\kappa.
	\end{split}
	\end{equation}
...
\setcounter{footnote}{0}
\begin{itemize}
	\item Having data $\left\langle \bm{x}, u, r, \bm{x}'\right\rangle\sim\bm{\mathcal{D}}$ from a Monte Carlo rollout available, an imporant problem is to estimate $a_{\pi_k}(\bm{x},\bm{u})$ in \eqref{eq:TRPO_opt_prob_sample}.
...
\begin{equation}
\label{eq:PPO_CSO}
		\El{\min\left\{\frac{\pi_{\bm{\theta}}(\bm{u}|\bm{x})}{\pi_{\bm{\theta}_k}(\bm{u}|\bm{x})}a_{\pi_k}(\bm{x},\bm{u}), \mathrm{clip}\left(\frac{\pi_{\bm{\theta}}(\bm{u}|\bm{x})}{\pi_{\bm{\theta}_k}(\bm{u}|\bm{x})}, 1-\epsilon, 1+\epsilon\right)a_{\pi_k}\right\}}{\bm{x}\sim\rho^{\pi_k}, \bm{u}\sim\pi_k}.
\end{equation}
\normalsize
\begin{itemize}
...
\vspace{0.1cm}
\begin{equation*}
		\max_{\bm{\theta}}\,\min\left\{\frac{\pi_{\bm{\theta}}(\bm{u}|\bm{x})}{\pi_{\bm{\theta}_k}(\bm{u}|\bm{x})}a_{\pi_k}(\bm{x},\bm{u}), \mathrm{clip}\left(\frac{\pi_{\bm{\theta}}(\bm{u}|\bm{x})}{\pi_{\bm{\theta}_k}(\bm{u}|\bm{x})}, 1-\epsilon, 1+\epsilon\right)a_{\pi_k}(\bm{u}|\bm{x})\right\}.
\end{equation*}
\begin{itemize}
...
\begin{itemize}
	...
	\begin{equation}
	\label{eq:PPO_AKP}
      \El{\frac{\pi_{\bm{\theta}}(\bm{u}|\bm{x})}{\pi_{\bm{\theta}_k}(\bm{u}|\bm{x})}a_{\pi_k}(\bm{x},\bm{u})-\beta D_{\mathrm{KL}}(\pi_{\bm{\theta}_k}(\cdot|\bm{x}) \parallel \pi_{\bm{\theta}}(\cdot|\bm{x}))}{\bm{x}\sim\rho^{\pi_k}, \bm{u}\sim\pi_k}.		
	\end{equation}
